(()=>{
  ...
  style.textContent=`
    ...
    body .toolbar{position:static!important;top:auto!important}
    .date-compare-table thead th{position:static!important;top:auto!important;left:auto!important}
    .date-compare-sticky-head{
      position:fixed;
      z-index:9000;
      overflow:hidden;
      pointer-events:none;
      ...
      border-top:0;
      box-shadow:0 4px 12px rgba(71,58,38,.08);
    }
    .date-compare-sticky-head-inner{position:relative;left:0;top:0}
    .date-compare-sticky-head table{border-collapse:separate;border-spacing:0;table-layout:fixed;background:#f8f4ec}
    .date-compare-sticky-head th{padding:10px 12px;border-bottom:1px solid #e7dfd3;font-size:12px;vertical-align:middle;background:#f8f4ec;text-align:center;font-weight:750;color:#354039}
    .date-compare-sticky-head th:first-child{text-align:left}
    .date-compare-sticky-head .period-name{font:700 17px/1.05 'Cormorant Garamond',serif;color:#2f3a33}
    .date-compare-sticky-head .period-months{display:block;margin-top:4px;font:500 10px/1.15 Inter,sans-serif;color:#777d76}
    ...
    .heat-cell{transition:background-color .2s ease}.cash-cell-split{display:block;margin-top:3px;color:#777d76;font-size:8.5px;font-weight:500;line-height:1.2;white-space:normal}.cash-cell-split strong{color:#59625c;font-weight:750}
  ...
  function splitLine(ooo,ip){return `<span class="cash-cell-split">ООО <strong>${fmtMoney(ooo)}</strong> · ИП <strong>${fmtMoney(ip)}</strong></span>`}
  function addCashMetric(tbody,label,cls,pds,totalGetter,oooGetter,ipGetter,important=false,key=''){
    const values=pds.map(pd=>pd.a?totalGetter(pd.a,pd.rows):null),styles=heatStyles(values,heatEnabled(label,key));
    const tr=document.createElement('tr');tr.className=cls+(important?' important':'');
    tr.innerHTML='<td>'+label+'</td>'+values.map((v,i)=>{const pd=pds[i],out=v===null||v===undefined?'<span class="empty">—</span>':fmtMoney(v),st=styles[i]?` style="background:${styles[i]}"`:'',split=pd.a?.cashComplete?splitLine(oooGetter(pd.a,pd.rows),ipGetter(pd.a,pd.rows)):'';return `<td class="heat-cell"${st}>${out}${split}</td>`}).join('');
    tbody.appendChild(tr);
  }
  ...
  window.renderPeriod=function(){
    ...
    note.textContent=any?'Периоды показаны рядом для сравнения. Факт — положительные поступления по ККМ ООО и ИП там, где загружен файл «Счета и оплаты».':'За выбранный год пока нет сохранённых данных.';
    addGroup(tbody,'Основные показатели','group-general',cols);addSub(tbody,'План и выполнение','sub-general',cols);
    addMetric(tbody,'План','row-general',pds,a=>a.plan,fmtMoney,false,'plan');addMetric(tbody,'Должно быть','row-general',pds,a=>a.due,fmtMoney,false,'due');addMetric(tbody,'Факт','row-general',pds,a=>a.factTotal,fmtMoney,true,'factTotal');addMetric(tbody,'Выставлено счетов','row-general',pds,a=>a.billedTotal,fmtMoney,false,'billedTotal');addMetric(tbody,'ДС ООО','row-general',pds,a=>a.cashComplete?a.cashOOO:null,fmtMoney,false,'cashOOO');addMetric(tbody,'ДС ИП','row-general',pds,a=>a.cashComplete?a.cashIP:null,fmtMoney,false,'cashIP');addMetric(tbody,'% выполнения плана месяца','row-general',pds,a=>a.execution,fmtPct,true,'execution');
    ...
    addGroup(tbody,'Стоматология','group-dent',cols);addSub(tbody,'Приёмы','sub-visits-dent',cols);addMetric(tbody,'Первичные','row-visits-dent',pds,a=>a.dentPrimary,fmtCount,false,'dentPrimary');addMetric(tbody,'Повторные','row-visits-dent',pds,a=>a.dentRepeat,fmtCount,false,'dentRepeat');addSub(tbody,'Выручка и экономика','sub-money-dent',cols);addCashMetric(tbody,'Выручка стоматологии','row-money-dent',pds,a=>a.dentRev,a=>a.dentOOO,a=>a.dentIP,true,'dentRev');addMetric(tbody,'Средний чек стоматологии','row-money-dent',pds,a=>a.dentAvg,fmtMoney,false,'dentAvg');addSub(tbody,'Выручка по врачам','sub-team-dent',cols);dentists.forEach(name=>addCashMetric(tbody,name,'row-team-dent',pds,a=>num((a.dentists||{})[name]),a=>num((a.cashDentistsOOO||{})[name]),a=>num((a.cashDentistsIP||{})[name]),false,'dentist'));
    addGroup(tbody,'Клиника','group-clinic',cols);addSub(tbody,'Приёмы','sub-visits-clinic',cols);addMetric(tbody,'Первичные','row-visits-clinic',pds,a=>a.clinicPrimary,fmtCount,false,'clinicPrimary');addMetric(tbody,'Повторные','row-visits-clinic',pds,a=>a.clinicRepeat,fmtCount,false,'clinicRepeat');addSub(tbody,'Выручка и экономика','sub-money-clinic',cols);addCashMetric(tbody,'Выручка клиники','row-money-clinic',pds,a=>a.clinicRev,a=>a.clinicOOO,a=>a.clinicIP,true,'clinicRev');addMetric(tbody,'Средний чек клиники','row-money-clinic',pds,a=>a.clinicAvg,fmtMoney,false,'clinicAvg');addSub(tbody,'Выручка по специалистам','sub-team-clinic',cols);clinicDocs.forEach(name=>addCashMetric(tbody,name,'row-team-clinic',pds,a=>num((a.clinicDocs||{})[name]),a=>num((a.cashClinicDocsOOO||{})[name]),a=>num((a.cashClinicDocsIP||{})[name]),false,'clinicDoctor'));
    addGroup(tbody,'Лаборатория','group-lab',cols);addSub(tbody,'Объём','sub-lab-volume',cols);addMetric(tbody,'Количество заказов','row-lab-volume',pds,a=>a.labOrders,fmtCount,false,'labOrders');addSub(tbody,'Выручка и экономика','sub-lab-money',cols);addCashMetric(tbody,'Выручка лаборатории','row-lab-money',pds,a=>a.labRevenue,a=>a.labOOO,a=>a.labIP,true,'labRevenue');addMetric(tbody,'Средний чек лаборатории','row-lab-money',pds,a=>a.labAvg,fmtMoney,false,'labAvg');
  ...
  const dateStickyHead=document.createElement('div');
  dateStickyHead.id='dateCompareStickyHead';
  dateStickyHead.className='date-compare-sticky-head hidden';
  dateStickyHead.setAttribute('aria-hidden','true');
  dateStickyHead.innerHTML='<div class="date-compare-sticky-head-inner"><table><colgroup></colgroup><thead></thead></table></div>';
  document.body.appendChild(dateStickyHead);
  const dateStickyInner=dateStickyHead.querySelector('.date-compare-sticky-head-inner');
  const dateStickyTable=dateStickyHead.querySelector('table');
  const dateStickyCols=dateStickyHead.querySelector('colgroup');
  const dateStickyThead=dateStickyHead.querySelector('thead');
  const dateCompareShell=dateCompareView.querySelector('.date-compare-table');
  ...
  function addDateCashMetric(tbody,label,cls,months,totalGetter,oooGetter,ipGetter,key='',important=false){
    const values=months.map(m=>m.r?totalGetter(m.d,m.r):null),styles=heatStyles(values,heatEnabled(label,key));
    const tr=document.createElement('tr');tr.className=cls+(important?' important':'');
    tr.innerHTML='<td>'+label+'</td>'+values.map((v,i)=>{const m=months[i],out=v===null?'<span class="empty">—</span>':fmtMoney(v),st=styles[i]?` style="background:${styles[i]}"`:'',split=m.d?.cashComplete?splitLine(oooGetter(m.d,m.r),ipGetter(m.d,m.r)):'';return `<td class="heat-cell"${st}>${out}${split}</td>`}).join('');
    tbody.appendChild(tr);
  }
  function recordDoctorTotal(r,type,name){if(r?.cashDataComplete===true){const key=type==='dent'?'cashDentists':'cashClinicDocs';return rawOrNull((cashMap(r,key)||{})[name])}const key=type==='dent'?'dentists':'clinicDocs';return rawOrNull((r?.[key]||{})[name])}
  function recordDoctorEntity(r,type,entity,name){if(r?.cashDataComplete!==true)return null;const key=type==='dent'?(entity==='ooo'?'cashDentistsOOO':'cashDentistsIP'):(entity==='ooo'?'cashClinicDocsOOO':'cashClinicDocsIP');return rawOrNull((cashMap(r,key)||{})[name])}
  function renderDateComparison(){
    ...
    note.textContent='Факт — положительные поступления по двум утверждённым ККМ. ООО/ИП показаны внутри денежных строк без увеличения числа строк.';const cols=months.length+1;
    addGroup(tbody,'Основные показатели','group-general',cols);addSub(tbody,'План и выполнение','sub-general',cols);addDateMetric(tbody,'План','row-general',months,(d,r)=>rawOrNull(r.plan),fmtMoney,'plan');addDateMetric(tbody,'Должно быть','row-general',months,d=>rawOrNull(d.due),fmtMoney,'due');addDateMetric(tbody,'Факт','row-general',months,d=>rawOrNull(d.factTotal),fmtMoney,'factTotal',true);addDateMetric(tbody,'Выставлено счетов','row-general',months,d=>rawOrNull(d.billedTotal),fmtMoney,'billedTotal');addDateMetric(tbody,'ДС ООО','row-general',months,d=>d.cashComplete?rawOrNull(d.cashOOO):null,fmtMoney,'cashOOO');addDateMetric(tbody,'ДС ИП','row-general',months,d=>d.cashComplete?rawOrNull(d.cashIP):null,fmtMoney,'cashIP');addDateMetric(tbody,'% выполнения плана месяца','row-general',months,d=>rawOrNull(d.execution),fmtPct,'execution',true);addDateMetric(tbody,'ПП 2025 на этот день','row-general',months,(d,r)=>rawOrNull(r.pp25),fmtCount,'pp25');addDateMetric(tbody,'Средний чек 2025','row-general',months,(d,r)=>rawOrNull(r.avg25),fmtMoney,'avg25');addSub(tbody,'Пациенты','sub-general',cols);addDateMetric(tbody,'Первичные приёмы','row-general',months,(d,r)=>rawOrNull(r.primary),fmtCount,'primary');addDateMetric(tbody,'Повторные приёмы','row-general',months,(d,r)=>rawOrNull(r.repeat),fmtCount,'repeat');addDateMetric(tbody,'ПП в день','row-general',months,d=>rawOrNull(d.ppDay),fmtCount,'ppDay');addDateMetric(tbody,'Средний чек медицины','row-general',months,d=>rawOrNull(d.avgCheck),fmtMoney,'avgCheck',true);
    addGroup(tbody,'Стоматология','group-dent',cols);addSub(tbody,'Приёмы','sub-visits-dent',cols);addDateMetric(tbody,'Первичные','row-visits-dent',months,(d,r)=>rawOrNull(r.dentPrimary),fmtCount,'dentPrimary');addDateMetric(tbody,'Повторные','row-visits-dent',months,(d,r)=>rawOrNull(r.dentRepeat),fmtCount,'dentRepeat');addDateMetric(tbody,'ПП в день','row-visits-dent',months,d=>rawOrNull(d.dentPPDay),fmtCount,'dentPPDay');addSub(tbody,'Выручка и экономика','sub-money-dent',cols);addDateCashMetric(tbody,'Выручка стоматологии','row-money-dent',months,d=>rawOrNull(d.dentRev),d=>rawOrNull(d.dentOOO),d=>rawOrNull(d.dentIP),'dentRev',true);addDateMetric(tbody,'Средний чек стоматологии','row-money-dent',months,d=>rawOrNull(d.dentAvg),fmtMoney,'dentAvg');addSub(tbody,'Выручка по врачам','sub-team-dent',cols);dentists.forEach(name=>addDateCashMetric(tbody,name,'row-team-dent',months,(d,r)=>recordDoctorTotal(r,'dent',name),(d,r)=>recordDoctorEntity(r,'dent','ooo',name),(d,r)=>recordDoctorEntity(r,'dent','ip',name),'dentist'));
    addGroup(tbody,'Клиника','group-clinic',cols);addSub(tbody,'Приёмы','sub-visits-clinic',cols);addDateMetric(tbody,'Первичные','row-visits-clinic',months,(d,r)=>rawOrNull(r.clinicPrimary),fmtCount,'clinicPrimary');addDateMetric(tbody,'Повторные','row-visits-clinic',months,(d,r)=>rawOrNull(r.clinicRepeat),fmtCount,'clinicRepeat');addDateMetric(tbody,'ПП в день','row-visits-clinic',months,d=>rawOrNull(d.clinicPPDay),fmtCount,'clinicPPDay');addSub(tbody,'Выручка и экономика','sub-money-clinic',cols);addDateCashMetric(tbody,'Выручка клиники','row-money-clinic',months,d=>rawOrNull(d.clinicRev),d=>rawOrNull(d.clinicOOO),d=>rawOrNull(d.clinicIP),'clinicRev',true);addDateMetric(tbody,'Средний чек клиники','row-money-clinic',months,d=>rawOrNull(d.clinicAvg),fmtMoney,'clinicAvg');addSub(tbody,'Выручка по специалистам','sub-team-clinic',cols);clinicDocs.forEach(name=>addDateCashMetric(tbody,name,'row-team-clinic',months,(d,r)=>recordDoctorTotal(r,'clinic',name),(d,r)=>recordDoctorEntity(r,'clinic','ooo',name),(d,r)=>recordDoctorEntity(r,'clinic','ip',name),'clinicDoctor'));
    addGroup(tbody,'Лаборатория','group-lab',cols);addSub(tbody,'Объём','sub-lab-volume',cols);addDateMetric(tbody,'Количество заказов','row-lab-volume',months,(d,r)=>rawOrNull(r.labOrders),fmtCount,'labOrders');addSub(tbody,'Выручка и экономика','sub-lab-money',cols);addDateCashMetric(tbody,'Выручка лаборатории','row-lab-money',months,d=>rawOrNull(d.labRevenue),d=>rawOrNull(d.labOOO),d=>rawOrNull(d.labIP),'labRevenue',true);addDateMetric(tbody,'Средний чек лаборатории','row-lab-money',months,d=>rawOrNull(d.labAvg),fmtMoney,'labAvg');
    ...
    queueDateStickyClone();
  }
  let dateStickyRaf=0;
  function hideDateStickyClone(){
    dateStickyHead.classList.add('hidden');
  }
  function syncDateStickyClone(){
    dateStickyRaf=0;
    const compare=document.getElementById('mode').value==='date'&&document.getElementById('dateViewMode').value==='compare';
    const shell=dateCompareShell,table=shell?.querySelector('table'),head=table?.querySelector('thead');
    const enabled=!!(compare&&window.innerWidth>900&&shell&&table&&head);
    if(!enabled){hideDateStickyClone();return;}
    const nav=document.querySelector('.az-section-nav');
    const navBottom=nav?Math.ceil(nav.getBoundingClientRect().bottom):0;
    const shellRect=shell.getBoundingClientRect(),tableRect=table.getBoundingClientRect(),headRect=head.getBoundingClientRect();
    const shouldShow=tableRect.top<navBottom&&tableRect.bottom>navBottom+Math.max(1,headRect.height);
    if(!shouldShow){hideDateStickyClone();return;}

    const sourceCells=[...head.querySelectorAll('th')];
    const widths=sourceCells.map(cell=>cell.getBoundingClientRect().width);
    const signature=head.innerHTML+'|'+widths.map(width=>width.toFixed(2)).join(',');
    if(dateStickyHead.dataset.signature!==signature){
      dateStickyThead.innerHTML=head.innerHTML;
      dateStickyCols.innerHTML=widths.map(width=>'<col style="width:'+width.toFixed(2)+'px">').join('');
      dateStickyHead.dataset.signature=signature;
    }

    dateStickyTable.style.width=Math.round(table.getBoundingClientRect().width)+'px';
    dateStickyInner.style.left=(-shell.scrollLeft)+'px';
    dateStickyHead.style.top=navBottom+'px';
    dateStickyHead.style.left=Math.round(shellRect.left)+'px';
    dateStickyHead.style.width=Math.round(shellRect.width)+'px';
    dateStickyHead.style.height=Math.ceil(headRect.height)+'px';
    dateStickyHead.classList.remove('hidden');
  }
  function queueDateStickyClone(){
    if(dateStickyRaf)return;
    dateStickyRaf=requestAnimationFrame(syncDateStickyClone);
  }
  ...
  window.addEventListener('scroll',queueDateStickyClone,{passive:true});
  window.addEventListener('resize',queueDateStickyClone,{passive:true});
  dateCompareShell?.addEventListener('scroll',queueDateStickyClone,{passive:true});
