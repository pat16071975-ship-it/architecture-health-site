(()=>{
  ...
  function addDateMetric(tbody,label,cls,months,getter,formatter,key='',important=false,detailGetter=null){
    const values=months.map(m=>m.r?getter(m.d,m.r):null),styles=heatStyles(values,heatEnabled(label,key));
    const tr=document.createElement('tr');tr.className=cls+(important?' important':'');tr.innerHTML='<td>'+label+'</td>'+values.map((v,i)=>{const detail=detailGetter&&months[i].r?detailGetter(months[i].d,months[i].r):'',out=v===null?'<span class="empty">—</span>':formatter(v)+(detail||''),st=styles[i]?` style="background:${styles[i]}"`:'';return `<td class="heat-cell"${st}>${out}</td>`}).join('');tbody.appendChild(tr);
  }
  function renderDateComparison(){
    ...
    addGroup(tbody,'Стоматология','group-dent',cols);addSub(tbody,'Приёмы','sub-visits-dent',cols);addDateMetric(tbody,'Первичные','row-visits-dent',months,(d,r)=>rawOrNull(r.dentPrimary),fmtCount,'dentPrimary');addDateMetric(tbody,'Повторные','row-visits-dent',months,(d,r)=>rawOrNull(r.dentRepeat),fmtCount,'dentRepeat');addDateMetric(tbody,'ПП в день','row-visits-dent',months,d=>rawOrNull(d.dentPPDay),fmtCount,'dentPPDay');addSub(tbody,'Выручка и экономика','sub-money-dent',cols);addDateMetric(tbody,'Выручка стоматологии','row-money-dent',months,d=>rawOrNull(d.dentRev),fmtMoney,'dentRev',true,(d,r)=>cashSplit({ooo:r.dentCashOOO,ip:r.dentCashIP}));addDateMetric(tbody,'Средний чек стоматологии','row-money-dent',months,d=>rawOrNull(d.dentAvg),fmtMoney,'dentAvg');addSub(tbody,'Выручка по врачам','sub-team-dent',cols);dentists.forEach(name=>addDateMetric(tbody,name,'row-team-dent',months,(d,r)=>rawOrNull((r.dentists||{})[name]),fmtMoney,'dentist',false,(d,r)=>cashSplit((r.dentistsLegal||{})[name])));
    addGroup(tbody,'Клиника','group-clinic',cols);addSub(tbody,'Приёмы','sub-visits-clinic',cols);addDateMetric(tbody,'Первичные','row-visits-clinic',months,(d,r)=>rawOrNull(r.clinicPrimary),fmtCount,'clinicPrimary');addDateMetric(tbody,'Повторные','row-visits-clinic',months,(d,r)=>rawOrNull(r.clinicRepeat),fmtCount,'clinicRepeat');addDateMetric(tbody,'ПП в день','row-visits-clinic',months,d=>rawOrNull(d.clinicPPDay),fmtCount,'clinicPPDay');addSub(tbody,'Выручка и экономика','sub-money-clinic',cols);addDateMetric(tbody,'Выручка клиники','row-money-clinic',months,d=>rawOrNull(d.clinicRev),fmtMoney,'clinicRev',true,(d,r)=>cashSplit({ooo:r.clinicCashOOO,ip:r.clinicCashIP}));addDateMetric(tbody,'Средний чек клиники','row-money-clinic',months,d=>rawOrNull(d.clinicAvg),fmtMoney,'clinicAvg');addSub(tbody,'Выручка по специалистам','sub-team-clinic',cols);clinicDocs.forEach(name=>addDateMetric(tbody,name,'row-team-clinic',months,(d,r)=>rawOrNull((r.clinicDocs||{})[name]),fmtMoney,'clinicDoctor',false,(d,r)=>cashSplit((r.clinicDocsLegal||{})[name])));
    addGroup(tbody,'Лаборатория','group-lab',cols);addSub(tbody,'Объём','sub-lab-volume',cols);addDateMetric(tbody,'Количество заказов','row-lab-volume',months,(d,r)=>rawOrNull(r.labOrders),fmtCount,'labOrders');addSub(tbody,'Выручка и экономика','sub-lab-money',cols);addDateMetric(tbody,'Выручка лаборатории','row-lab-money',months,(d,r)=>rawOrNull(r.labRevenue||r.factLab),fmtMoney,'labRevenue',true,(d,r)=>cashSplit({ooo:r.labCashOOO,ip:r.labCashIP}));addDateMetric(tbody,'Средний чек лаборатории','row-lab-money',months,d=>rawOrNull(d.labAvg),fmtMoney,'labAvg');
