(()=>{
  ...
  style.textContent=`
    ...
    .period-compare .row-secondary td,.date-compare-table .row-secondary td{font-size:10px;font-weight:600}
    .period-compare .row-billed td,.date-compare-table .row-billed td{background:#fff!important}
    .cash-split{display:block;margin-top:3px;font:500 8.5px/1.2 Inter,sans-serif;color:#737970;white-space:normal;letter-spacing:0}
  ...
  function cashSplit(raw){
    if(!raw||typeof raw!=='object')return '';
    const ooo=rawOrNull(raw.ooo),ip=rawOrNull(raw.ip);
    if(ooo===null&&ip===null)return '';
    if(num(ooo)+num(ip)<=0)return '';
    return `<span class="cash-split">ООО ${fmtMoney(num(ooo))}<br>ИП ${fmtMoney(num(ip))}</span>`;
  }
  function addDateCashMetric(tbody,label,cls,months,getter,key,important,splitGetter){
    const values=months.map(m=>m.r?getter(m.d,m.r):null),styles=heatStyles(values,heatEnabled(label,key));
    const tr=document.createElement('tr');tr.className=cls+(important?' important':'');
    tr.innerHTML='<td>'+label+'</td>'+values.map((v,i)=>{
      const split=months[i].r?cashSplit(splitGetter(months[i].d,months[i].r)):'',
        out=v===null?'<span class="empty">—</span>':fmtMoney(v)+(split||''),
        st=styles[i]?` style="background:${styles[i]}"`:'';
      return `<td class="heat-cell"${st}>${out}</td>`;
    }).join('');
    tbody.appendChild(tr);
  }
  function renderDateComparison(){
    ...
    addGroup(tbody,'Основные показатели','group-general',cols);addSub(tbody,'План и выполнение','sub-general',cols);addDateMetric(tbody,'План','row-general',months,(d,r)=>rawOrNull(r.plan),fmtMoney,'plan');addDateMetric(tbody,'Должно быть','row-general row-secondary',months,d=>rawOrNull(d.due),fmtMoney,'due');addDateMetric(tbody,'Факт','row-general',months,d=>rawOrNull(d.factTotal),fmtMoney,'factTotal',true);addDateMetric(tbody,'Выставлено счетов','row-general row-billed',months,d=>rawOrNull(d.billedTotal),fmtMoney,'billedTotal');addDateMetric(tbody,'ДС ООО','row-general row-secondary',months,d=>rawOrNull(d.cashOOO),fmtMoney,'cashOOO');addDateMetric(tbody,'ДС ИП','row-general row-secondary',months,d=>rawOrNull(d.cashIP),fmtMoney,'cashIP');addDateMetric(tbody,'% выполнения плана месяца','row-general',months,d=>rawOrNull(d.execution),fmtPct,'execution',true);addDateMetric(tbody,'ПП 2025 на этот день','row-general',months,(d,r)=>rawOrNull(r.pp25),fmtCount,'pp25');addDateMetric(tbody,'Средний чек 2025','row-general',months,(d,r)=>rawOrNull(r.avg25),fmtMoney,'avg25');addSub(tbody,'Пациенты','sub-general',cols);addDateMetric(tbody,'Первичные приёмы','row-general',months,(d,r)=>rawOrNull(r.primary),fmtCount,'primary');addDateMetric(tbody,'Повторные приёмы','row-general',months,(d,r)=>rawOrNull(r.repeat),fmtCount,'repeat');addDateMetric(tbody,'ПП в день','row-general',months,d=>rawOrNull(d.ppDay),fmtCount,'ppDay');addDateMetric(tbody,'Средний чек медицины','row-general',months,d=>rawOrNull(d.avgCheck),fmtMoney,'avgCheck',true);
    addGroup(tbody,'Стоматология','group-dent',cols);addSub(tbody,'Приёмы','sub-visits-dent',cols);addDateMetric(tbody,'Первичные','row-visits-dent',months,(d,r)=>rawOrNull(r.dentPrimary),fmtCount,'dentPrimary');addDateMetric(tbody,'Повторные','row-visits-dent',months,(d,r)=>rawOrNull(r.dentRepeat),fmtCount,'dentRepeat');addDateMetric(tbody,'ПП в день','row-visits-dent',months,d=>rawOrNull(d.dentPPDay),fmtCount,'dentPPDay');addSub(tbody,'Выручка и экономика','sub-money-dent',cols);addDateCashMetric(tbody,'Выручка стоматологии','row-money-dent',months,d=>rawOrNull(d.dentRev),'dentRev',true,(d,r)=>({ooo:r.dentCashOOO,ip:r.dentCashIP}));addDateMetric(tbody,'Средний чек стоматологии','row-money-dent',months,d=>rawOrNull(d.dentAvg),fmtMoney,'dentAvg');addSub(tbody,'Выручка по врачам','sub-team-dent',cols);dentists.forEach(name=>addDateCashMetric(tbody,name,'row-team-dent',months,(d,r)=>rawOrNull((r.dentists||{})[name]),'dentist',false,(d,r)=>(r.dentistsLegal||{})[name]));
    addGroup(tbody,'Клиника','group-clinic',cols);addSub(tbody,'Приёмы','sub-visits-clinic',cols);addDateMetric(tbody,'Первичные','row-visits-clinic',months,(d,r)=>rawOrNull(r.clinicPrimary),fmtCount,'clinicPrimary');addDateMetric(tbody,'Повторные','row-visits-clinic',months,(d,r)=>rawOrNull(r.clinicRepeat),fmtCount,'clinicRepeat');addDateMetric(tbody,'ПП в день','row-visits-clinic',months,d=>rawOrNull(d.clinicPPDay),fmtCount,'clinicPPDay');addSub(tbody,'Выручка и экономика','sub-money-clinic',cols);addDateCashMetric(tbody,'Выручка клиники','row-money-clinic',months,d=>rawOrNull(d.clinicRev),'clinicRev',true,(d,r)=>({ooo:r.clinicCashOOO,ip:r.clinicCashIP}));addDateMetric(tbody,'Средний чек клиники','row-money-clinic',months,d=>rawOrNull(d.clinicAvg),fmtMoney,'clinicAvg');addSub(tbody,'Выручка по специалистам','sub-team-clinic',cols);clinicDocs.forEach(name=>addDateCashMetric(tbody,name,'row-team-clinic',months,(d,r)=>rawOrNull((r.clinicDocs||{})[name]),'clinicDoctor',false,(d,r)=>(r.clinicDocsLegal||{})[name]));
    addGroup(tbody,'Лаборатория','group-lab',cols);addSub(tbody,'Объём','sub-lab-volume',cols);addDateMetric(tbody,'Количество заказов','row-lab-volume',months,(d,r)=>rawOrNull(r.labOrders),fmtCount,'labOrders');addSub(tbody,'Выручка и экономика','sub-lab-money',cols);addDateCashMetric(tbody,'Выручка лаборатории','row-lab-money',months,(d,r)=>rawOrNull(r.labRevenue||r.factLab),'labRevenue',true,(d,r)=>({ooo:r.labCashOOO,ip:r.labCashIP}));addDateMetric(tbody,'Средний чек лаборатории','row-lab-money',months,d=>rawOrNull(d.labAvg),fmtMoney,'labAvg');
