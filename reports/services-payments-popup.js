(() => {
  ...
  function installModal(w,d){
    ...
    const el=d.createElement('div');el.id='azPayModal';el.className='az-pay-backdrop az-pay-hide';el.innerHTML=`<div class="az-pay-modal"><div class="az-pay-head"><div><h3>ЗП и иные выплаты</h3><small id="azPayDoctor"></small></div><button class="az-pay-x" id="azPayX">×</button></div><div class="az-pay-body"><p class="az-pay-note">В «Иные выплаты» заноси отпускные, больничные, премии и другие доплаты. Пустые поля основной ЗП и доплат считаются нулём при расчёте.</p><div id="azPayTable"></div></div><div class="az-pay-actions"><button class="btn" id="azPayCancel">Отмена</button><button class="btn primary" id="azPaySave">Сохранить</button></div></div>`;d.body.appendChild(el);
  ...
  function open(w,d){
    ...
    const recalc=i=>{const a=parse(d.querySelector(`.az-base[data-i="${i}"]`)?.value,0),b=parse(d.querySelector(`.az-extra[data-i="${i}"]`)?.value,0);d.querySelector(`[data-t="${i}"]`).textContent=money((a||0)+(b||0))};d.querySelectorAll('.az-base,.az-extra').forEach(x=>x.oninput=()=>recalc(+x.dataset.i));
  ...
  function renderDoctorEconomics(w,d){
    ...
    box.innerHTML=`<div class="az-econ-head"><h2>Экономика врача — ${esc(doctor)}</h2><button id="azPayOpen" class="btn primary">ЗП и выплаты</button></div><div class="az-econ-note">Прибыль и маржинальность считаются после основной ЗП и иных выплат врачу, но до материалов, лаборатории и прочих расходов клиники. Пустые поля выплат считаются нулём.</div><div class="table-wrap"><table class="table"><thead><tr><th>Показатель</th>${groups.map(g=>`<th class="${focus===g.id?'focus':''}">${esc(g.label)}</th>`).join('')}<th class="total-col">Итого</th></tr></thead><tbody>${row('Выручка врача','revenue',money)}${row('Основная ЗП','salary',money)}${row('Иные выплаты','extras',money)}${row('Всего выплат врачу','total',money)}${row('Выплаты / выручка, %','share',percent)}${row('Прибыль после выплат врачу','profit',money,true)}${row('Маржинальность после выплат, %','margin',percent,true)}</tbody></table></div>`;d.getElementById('azPayOpen').onclick=()=>open(w,d);
    const idx=focusIdx(data,d),e=econ(dir,doctor,salary,extras,idx),k=d.getElementById('kpis');if(k)k.innerHTML=`<div class="kpi"><span>Выручка врача</span><strong>${money(e.revenue)}</strong></div><div class="kpi"><span>Всего выплат врачу</span><strong>${money(e.total)}</strong></div><div class="kpi"><span>Прибыль после выплат</span><strong class="${cls(e.profit)}">${money(e.profit)}</strong></div><div class="kpi"><span>Маржинальность</span><strong class="${cls(e.margin)}">${e.margin==null?'—':percent(e.margin)}</strong></div>`;
  }
  ...
  function econRow(name,e){return`<tr><td>${esc(name)}</td><td>${money(e.revenue)}</td><td>${money(e.salary)}</td><td>${money(e.extras)}</td><td>${money(e.total)}</td><td class="${cls(e.profit)}">${money(e.profit)}</td><td class="${cls(e.margin)}">${e.margin==null?'—':percent(e.margin)}</td></tr>`}
